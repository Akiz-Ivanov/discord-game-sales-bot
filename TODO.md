...
- [x] `/about` command + clickable command mentions + bot renamed to
      "PC Game Deals". New `discord/views/about.ts`
      (`buildAboutMessage`) — Components V2, ephemeral, teal accent
      (`0x1abc9c`, unused elsewhere). Covers what the bot does, credits
      IsThereAnyDeal and GamerPower by name with a one-line description
      of what each actually does (not just a bare name — meant to
      genuinely drive interest toward both), a `/feedback` mention, and
      a Link-style button to the GitHub repo. Considered crediting
      icon/font/banner assets (Tabler/Phosphor icons, a Freepik banner)
      alongside ITAD/GamerPower — deliberately left out: those are
      presentation assets nobody using `/about` cares about, a
      different category from actual data-source credit. Confirmed via
      both services' ToS that logo usage isn't granted by their terms
      (ITAD: silent on logos; GamerPower: explicit "no rights to our
      IP beyond API access") — text + hyperlink only, no logos used.
  - New `discord/interactions/commandMention.ts`'s `mention()` helper
    — renders Discord's native `</name:id>` (or `</name sub:id>` for
    subcommands) syntax, which renders as a clickable pill that inserts
    the command scaffold into the sender's chatbox on click. Can't
    pre-fill parameter values — Discord exposes no API for that, only
    the scaffold up to the subcommand. `register-commands.js` now
    writes the `PUT` response's per-command ids out to a generated
    `src/discord/commandIds.ts` (real committed source file, not a
    build artifact — imported by `mention()`), regenerated by re-running
    `npm run register-commands` any time a command is added/renamed.
  - `/help`'s header simplified from a repeated name+tagline (already
    redundant with the "PC Game Deals APP" badge Discord renders above
    every reply) down to a plain `### Commands` — orientation, not a
    re-pitch. All eight command entries converted to clickable mentions.
    `/privacy-policy`'s "How to remove it" section and the scattered
    fallback/error strings in `wishlist.ts`/`config.ts`/`price.ts`
    components/commands also converted where the message serves a
    discovery purpose (`/help`, `/privacy-policy`); left as plain
    backtick text where it's a transient error a user just triggered
    by running that exact command seconds earlier (stale-selection
    errors, empty-wishlist notices) — the mention's discovery value
    doesn't apply there.
  - Bot display name changed in Discord Developer Portal from the old
    working name to "PC Game Deals" (no "Bot" suffix — the `APP` badge
    Discord already renders is the "this is a bot" signal, redundant to
    repeat in the name itself). New `BOT_NAME` constant in
    `lib/constants.ts`, replacing hardcoded old-name strings in
    `welcome.ts` (both rich/lean headers) and `/privacy`'s page
    metadata + intro paragraph — a future rename now touches one
    constant instead of a grep-and-hope pass. Welcome card's rich
    header also renamed from a bot-name repeat to "Getting Started"
    (title case), lean/ephemeral variant to "Quick access" — same
    redundant-badge reasoning as `/help`.
  - Welcome card's utility row gained a third button (About, between
    Help and Feedback) — `handleWelcomeAbout` component handler,
    mirrors `handleWelcomeHelp`'s shape exactly (sync, delegates
    straight to `buildAboutMessage()`).
  - Full test coverage: `views/about.test.ts`, `commands/about.test.ts`,
    `interactions/commandMention.test.ts` (all new), updated
    `views/help.test.ts` (header + eighth command entry),
    `views/welcome.test.ts` (three-button utility row assertion),
    `components/welcome.test.ts` (new `handleWelcomeAbout` test), and
    `test/e2e/welcome.e2e.test.ts` (updated header-content assertion
    for the renamed card). 487/487 passing project-wide, ~98.6%
    coverage.
  - **Deliberately deferred, unchanged from prior scoping**: bot
    latency/ping stat, a plain `/changelog` page (explicitly not
...
- [ ] `/about` polish — deferred additions, not urgent:
  ...
  - Changelog — NOT as GitHub Releases/tags (adds a CI/tagging workflow
    ...
    Vercel deploy, same pattern as `/privacy` — static route, no client
    JS, reverse-chronological plain-English entries lightly adapted
    from each session's TODO.md "what shipped" bullets rather than
    written fresh. `/about` would link to it via the same Link-button
